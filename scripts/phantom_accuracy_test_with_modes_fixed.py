#!/usr/bin/env python3
# phantom_accuracy_test.py
#
# 3-cam step phantom accuracy workflow (phantom-frame reporting + "measure all 47" mode)
#
# Flow:
# 1) Acquire a synchronized triplet (from live cameras or explicit files).
# 2) Register phantom pose by precisely clicking:
#       - Fiducial 1  (origin)
#       - Fiducial 19 (defines +X from 1)
#       - A third anchor (default ID=7) to fully fix rotation
# 3) Choose mode:
#       (A) Arbitrary IDs: enter any fiducial 1–47 repeatedly
#       (B) All 47 in order: the script prompts through 1→47; you can skip a point
# 4) For each fiducial:
#       - Two-click ROI→zoom picking on cam0→cam1→cam2
#       - Triangulate 3D in cam0 frame
#       - Map MEASURED point to PHANTOM frame (using inverse registration)
#       - Compare to validated phantom coordinates (Reference), report ΔX,ΔY,ΔZ, |Δ|, reprojection RMS
#       - Log to CSV; save overlays (optional)
#
# Intrinsics: intrinsics_cam0.npz, intrinsics_cam1.npz, intrinsics_cam2.npz
# Rig      : rig_final.npz or rig_final.json  (must contain H_cam1_in_cam0, H_cam2_in_cam0)
# Phantom  : phantom_points.xlsx/csv with columns: id, x, y, z   (PHANTOM FRAME)
#
# Example (LIVE):
#   python phantom_accuracy_test.py --live --cams 0 1 2 \
#     --intrinsics intrinsics_cam0.npz intrinsics_cam1.npz intrinsics_cam2.npz \
#     --rig rig_final.npz \
#     --phantom phantom_points.xlsx \
#     --out_root captures --csv_out phantom_accuracy.csv \
#     --roi_half 50 --magnification 10 --save_debug
#
# Example (FILES):
#   python phantom_accuracy_test.py \
#     --images capture0/cam0_XXXX.png capture1/cam1_XXXX.png capture2/cam2_XXXX.png \
#     --intrinsics intrinsics_cam0.npz intrinsics_cam1.npz intrinsics_cam2.npz \
#     --rig rig_final.json \
#     --phantom phantom_points.xlsx \
#     --csv_out phantom_accuracy.csv --roi_half 50 --magnification 10 --save_debug
#
...
# --- Accuracy modes add-on ---
try:
    from acc_modes import (
        distance_mode_summary, bestfit_holdout_summary,
        select_auto_pairs, select_auto_anchor_holdout
    )
except Exception as _e:
    distance_mode_summary = bestfit_holdout_summary = None
    select_auto_pairs = select_auto_anchor_holdout = None
    print("[WARN] acc_modes not found; accuracy modes disabled:", _e)
# ---------------- IO helpers ----------------
...
class TwoClickPicker:
    ...
    Two-click ROI→zoom picker with:
      ...
      - Robust slider handling via per-frame polling (fixes 'slider has no effect')
      - 16-bit ROI safe (auto-normalizes to 8-bit for display/thresholding)

    Usage:
        picker = TwoClickPicker(img, title="cam0", roi_half=50, magnification=10, subpixel=True)
        (x, y) = picker.run_once()
    """
    ...
    def _ensure_u8(self, roi):
        """Ensure uint8 ROI for display/threshold; normalize if 16-bit/float."""
    ...
    def _draw_grid(self, canvas, pane_origin=(0,0), roi_shape=None):
        ...
        H, W = roi_shape
        ox, oy = pane_origin
        step_roi = 5     # grid every 5 ROI px
        # verticals
    ...
    def _compose_zoom_view(self, roi_u8, mode, tval, invert, out_otzu_val=None):
        """
        roi_u8: uint8 ROI
        mode: 0=Auto(Otsu), 1=Manual
        tval: threshold value (0..255) if Manual
        invert: 0/1
        out_otzu_val: optional list of length 1 to store the Otsu threshold used
        Returns: vis(BGR), bin_mask(uint8 ROI scale)
        """
        if mode == 0:
    ...
    # ---------- zoom UI with per-frame slider polling ----------
    def _zoom_pick(self, roi_gray):
        """
        Show zoom with Method/Threshold/Invert sliders and grid toggle.
        Returns (rx, ry) in ROI coords (float), or None on cancel.
        """
        win = self.title_zoom
        cv2.namedWindow(win, cv2.WINDOW_NORMAL)

        # Ensure uint8 ROI (handles 16-bit/float cases)
        roi_u8 = self._ensure_u8(roi_gray)

        # Create trackbars (values will be polled each frame)
        ...
        last_mode = last_thr = last_inv = None
        otzu_val_holder = [None]  # to show Auto threshold value

        while True:
            # Poll sliders each frame (robust)
            ...
            if clicked["pt"] is not None:
                zx, zy = clicked["pt"]
                if zx >= pane_w:
                    zx -= pane_w  # if clicked right pane, wrap into left pane coords
    ...
    def run_once(self):
        """
        1) Full image window: left-click selects ROI → opens zoom UI
        2) Zoom UI: adjust threshold (Auto/Manual/Invert), grid toggle, click to confirm point
        3) Back in full window: nudge with arrows or h/j/k/l; '['/']' adjust step
           Press 'q' or Enter to accept; 'r' to redo.
        Returns (x,y) in full-image coords (sub-pixel refined if enabled).
        """
        ...
        def main_mouse_cb(event, x, y, flags, userdata):
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                if res is not None:
                    ...
                    chosen["pt"] = [fx, fy]  # mutable for nudging
        ...
        while True:
            ...
            if key in (ord('q'), 13):  # q or Enter
                ...
                # Arrow keys (OpenCV keycodes 81/82/83/84) + vim (h/j/k/l)
...
def invert_rt(R, t):
    """Return inverse of transform x' = R x + t  →  x = R^T (x' - t)."""
    Ri = R.T
    ti = -Ri @ t
    return Ri, ti
...
def open_streams(cam_ids):
    # Try user's CameraStream; fallback to OpenCV VideoCapture
    try:
        from cam3_stream import CameraStream
        streams = {}
        for cid in cam_ids:
...
# ---------------- Main ----------------
def main():
    ap = argparse.ArgumentParser(description="Register step phantom (1,19,+anchor), then measure fiducials with 3-view triangulation. Reports in PHANTOM frame.")
    ...
    ap.add_argument("--intrinsics", nargs=3, required=True, help="intrinsics_cam0.npz intrinsics_cam1.npz intrinsics_cam2.npz")
    ...
    ap.add_argument("--anchor_id", type=int, default=18, help="Third non-collinear fiducial (default 18)")
    ...
    ap.add_argument("--acc_mode", choices=["dist","fit","both"], default="dist",                    help="Accuracy evaluation: pose-free distances, best-fit holdout, or both")
    ap.add_argument("--anchors", nargs="+", type=int, default=[],                    help="Anchor IDs for best-fit (if empty, auto-select)")
    ap.add_argument("--holdout", nargs="+", type=int, default=[],                    help="Hold-out IDs (if empty, all non-anchors)")
    ap.add_argument("--pairs", nargs="+", type=str, default=[],                    help="Explicit distance pairs like 1-19,7-18")
    ap.add_argument("--skip_registration", action="store_true",                    help="Skip 3-point phantom registration (for pose-free distance mode)")
    ap.add_argument("--plots", action="store_true", help="Save simple matplotlib plots")
    ...
    # CSV (PHANTOM-frame reporting)
    header = ["stamp","fid_id",
              "u0","v0","u1","v1","u2","v2",
              "X_meas_ph","Y_meas_ph","Z_meas_ph",     # measured, mapped to phantom frame
              "X_ref_ph","Y_ref_ph","Z_ref_ph",       # phantom validated coords (reference)
              "dX","dY","dZ","err_norm",
    ...
    if not args.skip_registration:
        print("\n=== Registration ===")
        print("Click (two-click ROI→zoom per camera, in order cam0→cam1→cam2):")
        print("  1) Fiducial 1  (origin)")
        print("  2) Fiducial 19 (on +X from 1)")
        print(f"  3) Fiducial {args.anchor_id} (third anchor to lock rotation)")
        
        clicks = click_triplet("fiducial 1 (origin)");      assert clicks is not None
        X1_cam, _, _, = tri_from_pixels(*clicks)
        clicks = click_triplet("fiducial 19 (x-axis)");     assert clicks is not None
        X19_cam, _, _, = tri_from_pixels(*clicks)
        clicks = click_triplet(f"fiducial {args.anchor_id} (anchor)"); assert clicks is not None
        Xa_cam, _, _, = tri_from_pixels(*clicks)
        
        P_src = np.vstack([P(1), P(19), P(args.anchor_id)])   # phantom frame
        Q_dst = np.vstack([X1_cam, X19_cam, Xa_cam])          # cam0 frame
        R_ph2c, t_ph2c = kabsch(P_src, Q_dst)                 # phantom → cam0
        R_c2ph, t_c2ph = invert_rt(R_ph2c, t_ph2c)            # cam0 → phantom   (used for reporting)
        
        # quick sanity on x-axis alignment (not used in computation)
        def angle_between(a,b):
            a=a/np.linalg.norm(a); b=b/np.linalg.norm(b)
            return np.degrees(np.arccos(np.clip(np.dot(a,b), -1, 1)))
        print("\nRegistration complete (phantom→cam0).")
        print(f"Sanity: |P(19)-P(1)|={np.linalg.norm(P(19)-P(1)):.3f}, |X19-X1|={np.linalg.norm(X19_cam-X1_cam):.3f}, "
        f"axis angle ≈ {angle_between((P(19)-P(1)), (R_c2ph @ (X19_cam-X1_cam))):.2f}°")
    else:
        print("\n=== Registration === (skipped via --skip_registration)")

    meas_pts = {}
    ids_clicked = []
    # -------------- Mode selection --------------
    print("\n=== Choose measurement mode ===")
    print("  A) Arbitrary IDs (enter 1–47 as you wish)")
    print("  B) All 47 in order (1 → 47, with option to skip)")
    mode = input("Select mode [A/B]: ").strip().lower()
    all_mode = (mode == "b")

    
    def measure_one(fid):
            """Click fiducial fid, triangulate; optionally map to phantom frame, log & print."""
            ...
            # Store for post-run accuracy modes
            meas_pts[fid] = X_cam.copy(); ids_clicked.append(fid)

            print(f"\nFID {fid:02d}")
            if not args.skip_registration:
                # Map to phantom frame and compare to truth
                X_meas_ph = R_c2ph @ X_cam + t_c2ph
                X_ref_ph = P(fid)  # reference in phantom frame
                d = X_meas_ph - X_ref_ph
                err_norm = float(np.linalg.norm(d))
                print("  (PHANTOM frame reporting)")
                ...
                print("  (Pose-free mode; skipping phantom-frame comparison)")
            print(f"  Reproj RMS: {rms:.3f}   cam0:{e0:.3f}  cam1:{e1:.3f}  cam2:{e2:.3f}")
            # CSV logging: guard phantom-only fields when skipping registration
            if not args.skip_registration:
                writer.writerow([time.time(), fid, X_meas_ph[0], X_meas_ph[1], X_meas_ph[2],
                            X_ref_ph[0], X_ref_ph[1], X_ref_ph[2],
                            d[0], d[1], d[2], err_norm, u0,v0,u1,v1,u2,v2, e0,e1,e2, rms])
            else:
                writer.writerow([time.time(), fid, X_cam[0], X_cam[1], X_cam[2],
                            "", "", "", "", "", "", "", u0,v0,u1,v1,u2,v2, e0,e1,e2, rms])
            if args.save_debug:
                # Existing debug image saving should still work; leave as-is or add pose-free overlay as needed
                pass
            return True


            print(f"\nFID {fid:02d}  (PHANTOM frame reporting)")
            print(f"  Measured_ph:  [{X_meas_ph[0]:.3f}, {X_meas_ph[1]:.3f}, {X_meas_ph[2]:.3f}]")
            print(f"  Reference_ph: [{X_ref_ph[0]:.3f}, {X_ref_ph[1]:.3f}, {X_ref_ph[2]:.3f}]")
            print(f"  Δ = [{d[0]:.3f}, {d[1]:.3f}, {d[2]:.3f}]   |Δ| = {err_norm:.3f}")
            print(f"  Reproj RMS (px): {rms:.2f}   (cam0 {e0:.2f}, cam1 {e1:.2f}, cam2 {e2:.2f})")

            writer.writerow([stamp, fid,
                             u0, v0, u1, v1, u2, v2,
    ...
    if all_mode:
        ...
        for fid in range(1, 48):
            ...
            ok = measure_one(fid)
            if not ok:
                # Allow retry/skip after a cancel
    ...
    # -------- Accuracy modes evaluation (pose-free distances and/or best-fit holdout) --------
    if distance_mode_summary is None:
        print("[INFO] Accuracy modes unavailable (acc_modes not imported).")
    else:
        import csv as _csv
        # Build phantom lookup
        id2idx = {int(id[i]): i for i in range(len(id))}
        # Pose-free distances
        if args.acc_mode in {"dist","both"}:
            rows_dist, stats_dist = distance_mode_summary(meas_pts, phantom_xyz, id2idx, ids_clicked, explicit_pairs=args.pairs)
            print("\n[DIST] summary:", stats_dist)
            dist_csv = os.path.splitext(args.csv_out)[0] + "_dist.csv"
            with open(dist_csv, "w", newline="", encoding="utf-8") as fdist:
                w = _csv.writer(fdist)
                w.writerow(["i","j","d_ref_mm","d_meas_mm","d_err_mm"])
                for i,j,dr,dm,de in rows_dist:
                    w.writerow([i,j,f"{dr:.6f}",f"{dm:.6f}",f"{de:.6f}"])
                w.writerow([]); w.writerow(["SUMMARY", json.dumps(stats_dist)])
            print(f"[DIST] wrote {dist_csv}")
            if args.plots and rows_dist:
                import matplotlib.pyplot as plt
                errs = [r[-1] for r in rows_dist]
                plt.figure(); plt.hist(errs, bins=30); plt.title("Distance errors (mm)"); plt.xlabel("d_meas - d_ref (mm)"); plt.ylabel("count"); plt.savefig(os.path.splitext(args.csv_out)[0]+"_dist_hist.png", dpi=150); plt.close()
        # Best-fit with holdout
        if args.acc_mode in {"fit","both"}:
            anchors = args.anchors or (select_auto_anchor_holdout(ids_clicked)[0] if select_auto_anchor_holdout else [])
            holdout = args.holdout or [i for i in ids_clicked if i not in anchors]
            rows_fit, stats_fit = bestfit_holdout_summary(meas_pts, phantom_xyz, id2idx, anchors=anchors, holdout=holdout)
            print("\n[FIT] summary:", stats_fit, " anchors:", anchors, " holdout:", holdout[:10], ("... (%d more)"%max(0,len(holdout)-10)))
            fit_csv = os.path.splitext(args.csv_out)[0] + "_fit.csv"
            with open(fit_csv, "w", newline="", encoding="utf-8") as ffit:
                w = _csv.writer(ffit)
                w.writerow(["fid","dx_mm","dy_mm","dz_mm","norm_mm"])
                for fid,dx,dy,dz,nm in rows_fit:
                    w.writerow([fid,f"{dx:.6f}",f"{dy:.6f}",f"{dz:.6f}",f"{nm:.6f}"])
                w.writerow([]); w.writerow(["SUMMARY", json.dumps(stats_fit)])
            print(f"[FIT] wrote {fit_csv}")
            if args.plots and rows_fit:
                import matplotlib.pyplot as plt
                norms = [r[-1] for r in rows_fit]
                plt.figure(); plt.hist(norms, bins=30); plt.title("|Δ| after best-fit (mm)"); plt.xlabel("mm"); plt.ylabel("count"); plt.savefig(os.path.splitext(args.csv_out)[0]+"_fit_hist.png", dpi=150); plt.close()
